...
class Ui_Info_Editor(object):
    ...
    def modding(self):
        ...
        self.Save_change_button.clicked.connect(lambda: Info_Editor_function.press_save())
...
class Ui_Concentrate_Advance(object):
    ...
    def modding(self):
        ...
        # connect button to function
        self.Show_all_button.clicked.connect(lambda: helper.set_current_search_type(Search_Edit_function, "all"))
        self.Show_all_button.clicked.connect(lambda: Search_Edit_function.show_user(0))
        self.ID_search_button.clicked.connect(lambda: helper.set_current_search_type(Search_Edit_function, "id"))
        self.ID_search_button.clicked.connect(lambda: Search_Edit_function.show_user(0))
        self.Name_search_button.clicked.connect(lambda: helper.set_current_search_type(Search_Edit_function, "name"))
        self.Name_search_button.clicked.connect(lambda: Search_Edit_function.show_user(0))
        self.Gender_button.clicked.connect(lambda: helper.set_current_search_type(Search_Edit_function, "gender"))
        self.Gender_button.clicked.connect(lambda: Search_Edit_function.show_user(0))
        self.Permission_button.clicked.connect(lambda: helper.set_current_search_type(Search_Edit_function, "permission"))
        self.Permission_button.clicked.connect(lambda: Search_Edit_function.show_user(0))
        self.Save_change_button.clicked.connect(lambda: Search_Edit_function.press_save())
    ...
    # Modding for Login History
        ...
        # connect button to function
        self.Show_all_button_login_history.clicked.connect(lambda: helper.set_current_search_type(Login_History_function, "all"))
        self.Show_all_button_login_history.clicked.connect(lambda: Login_History_function.show_login_history(0))
        self.ID_search_button_login_history.clicked.connect(lambda: helper.set_current_search_type(Login_History_function, "id"))
        self.ID_search_button_login_history.clicked.connect(lambda: Login_History_function.show_login_history(0))
        self.Status_show_button.clicked.connect(lambda: helper.set_current_search_type(Login_History_function, "status"))
        self.Status_show_button.clicked.connect(lambda: Login_History_function.show_login_history(0))
        self.Fail_type_button.clicked.connect(lambda: helper.set_current_search_type(Login_History_function, "specific_type_fail"))
        self.Fail_type_button.clicked.connect(lambda: Login_History_function.show_login_history(0))
        self.Success_type_button.clicked.connect(lambda: helper.set_current_search_type(Login_History_function, "specific_type_success"))
    ...
    # Modding for plate info
        ...
        self.Show_all_button_Plate_Info.clicked.connect(lambda: helper.set_current_search_type(Plate_Info_function, "all"))
        self.Show_all_button_Plate_Info.clicked.connect(lambda: Plate_Info_function.show_plate_info(0))
        self.ID_search_button_Plate_Info.clicked.connect(lambda: helper.set_current_search_type(Plate_Info_function, "user_id"))
        self.ID_search_button_Plate_Info.clicked.connect(lambda: Plate_Info_function.show_plate_info(0))
        self.Plate_ID_search_button_Plate_Info.clicked.connect(lambda: helper.set_current_search_type(Plate_Info_function, "plate_id"))
        self.Plate_ID_search_button_Plate_Info.clicked.connect(lambda: Plate_Info_function.show_plate_info(0))
        self.Availability_button.clicked.connect(lambda: helper.set_current_search_type(Plate_Info_function, "availability"))
        ...
        self.Deassign_plate_from_user.clicked.connect(lambda: Plate_Info_function.show_confirm_window("Deassign confirm", "Deassign these plates from Users?", Plate_Info_function.plate_action(0, None)))    
        self.Remove_plate.clicked.connect(lambda: Plate_Info_function.show_confirm_window("Remove confirm", "Remove these plates from database?", Plate_Info_function.plate_action(1, None)))
...
class UI_Login_function:
    ...
    def login(self, userid, password):
        ...
        ID_exist = current_user.exist

        if (ID_password_match is True):
            # enter Advance mode
            if (ID_is_admin is True and Loginui.checkBox.isChecked()):
                MySQL_func.add_login_record(current_user.userid, "SUCCESS", "Login with Advance mode")
            ...
            # Fail: Not admin but try to entering advance mode
                ...
                MySQL_func.add_login_record(current_user.userid, "FAIL", "A normal user tries to enter Advance mode")
            ...
            # enter Normal user mode
                ...
                Login.close()
                MySQL_func.add_login_record(current_user.userid, "SUCCESS", "Login with normal user mode")
                return True
        # Fail: ID doesn't exist
        elif (ID_exist is False):
            MySQL_func.add_login_record(current_user.userid, "FAIL", "ID doesn't exist")
        ...
        # Fail: ID doesn't match password
            ...
            MySQL_func.add_login_record(current_user.userid, "FAIL", "ID and Password doesn't match")
            # display the error message
    ...
    def open_normal_welcome_message(self):
        ...
        confirm.accepted.connect(lambda: Info_Editor_function.show_user(0))
        x = confirm.exec_()

# Parent class for user info edit functions
class User_info_edit_function:
    def __init__(self):
        self.check_button_array = []
        self.current_user = user(None)
        self.table = None
        
    def set_table(self, table, current_search_type, value, pages, *column_to_lock):
        data = MySQL_func.get_data_from_database("User_Information", current_search_type, value, pages)
        ...
        for i in range(table.rowCount()):
            table.setItem(i, 0, QtWidgets.QTableWidgetItem(None))
        ...
        # lock the column
        for column in column_to_lock:
            helper.lock_the_Column(table, column)

    def save_info(self, table):
        # start_time = time.time()

        self.permission_valid = helper.check_if_table_has_valid_data_only(table, self)[0]
        self.gender_valid = helper.check_if_table_has_valid_data_only(table, self)[1]
        self.password_valid = helper.check_if_table_has_valid_data_only(table, self)[2]

        if(self.permission_valid and self.gender_valid and self.password_valid):
            for row in range(table.rowCount()):
                if (self.check_button_array[row].isChecked()):                        
                    userid_list_on_table = helper.get_userid_on_table_list(table)
                    current_checking_user = user(userid_list_on_table[row])
                    self.current_edited_row = helper.get_current_edited_row(table, row)
                    MySQL_func.update_row_to_database(self.current_edited_row)
                    helper.compare_row_with_user(self.current_edited_row, current_checking_user, AdminEditorLogger, self.current_user)
        # AdminEditorLogger.logger.info("Save action finished --- %s seconds ---" % (time.time() - start_time))        

class UI_Info_Editor_function(User_info_edit_function):    
    def show_user(self, pages):
        ...
        self.set_table(self.table, "id_strict", self.current_user.userid, pages, 1, 3, 6)

    def press_save(self):
        self.save_info(self.table)
        Login_function.Info_Editor_ui.Permission_Invalid.setHidden(self.permission_valid)
        Login_function.Info_Editor_ui.Gender_Invalid.setHidden(self.gender_valid)
        Login_function.Info_Editor_ui.Password_Invalid.setHidden(self.password_valid) 
        self.show_user(0) 

class UI_Search_Edit_function(User_info_edit_function):
    ...
    def show_user(self, pages):
        ...
        if (self.current_search_type == "id"):
            if (Login_function.Concentrate_Advance_ui.ID_input.text() == None):
                self.value = ""
            self.value = Login_function.Concentrate_Advance_ui.ID_input.text()
        elif (self.current_search_type == "name"):
            if (Login_function.Concentrate_Advance_ui.Name_input.text() == None):
                self.value = ""
            self.value = Login_function.Concentrate_Advance_ui.Name_input.text()
...
class UI_Plate_info_function:
    ...
    def show_plate_info(self, pages):
        ...
        helper.insert_checkbox(self, self.table, data)
    ...
    # confirm window for delete and deassign plate
    def show_confirm_window(self, title, text, action):
        confirm = QMessageBox()
        confirm.setWindowTitle(title)
        confirm.setText(text)
        confirm.setStandardButtons(QMessageBox.Cancel | QMessageBox.Ok)
        confirm.setDefaultButton(QMessageBox.Cancel)
        confirm.accepted.connect(lambda: action)
        x = confirm.exec_()        
