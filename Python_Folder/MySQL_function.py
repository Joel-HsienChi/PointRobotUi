    ...
    def get_data_from_table(self, display_table, function_class):
        ...
        for row in range(display_table.rowCount()):
            ...
            if (function_class.check_button_array[row].isChecked()):
                if (not (edited_row[3] == "USER" or edited_row[3] == "ADMIN")):
                    Permission_fit = False
                if (not (edited_row[5] == "MALE" or edited_row[5] == "FEMALE" or edited_row[5] == "OTHERS")):
                    Gender_fit = False
                if (not (edited_row[2] == "************")):
                    if (not (helper_function_class().check_password_availability(edited_row[2]))):
                        Password_fit = False
                if (Gender_fit and Permission_fit and Password_fit):
                    if (edited_row[2] == "************"):
                        self.update_data_to_user_info(
                            edited_row[1], None, edited_row[3], edited_row[4], edited_row[5], edited_row[6])
                    else:
                        self.update_data_to_user_info(edited_row[1], helper_function_class().encode_password(edited_row[2]), edited_row[3], edited_row[4], edited_row[5], edited_row[6])

            self.update_plate_user_have(edited_row[1])
